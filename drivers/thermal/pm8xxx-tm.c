...
extern int get_battery_temperature(void);
#endif
#endif
...
#ifndef CONFIG_LGE_PM_THERMALD_FROM_BATTERY	
	*temp = adc_result.physical;
	chip->temp = adc_result.physical;
#else
	battery_temp = 1000 * (long)get_battery_temperature();
	*temp = battery_temp;
	chip->temp = battery_temp;
#endif
