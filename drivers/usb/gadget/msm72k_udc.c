...
#elif defined(CONFIG_MACH_LGE_C1_BOARD_MPS)
  if((0<=acc_read_value)&&(acc_read_value<=70))
        {
                cable_type = MHL_CABLE_500MA;
        }
...
static acc_cable_type get_usb_cable_type(void)
{
...
#if defined(CONFIG_MACH_LGE_I_BOARD_VZW)
	if((200<=acc_read_value)&&(acc_read_value<=600)) // 56k ==>LT cable, maximum current
	{
	...
	else if((1728<=acc_read_value)&&(acc_read_value<=1878)) // 910k ==>LT cable, maximum current
	{
