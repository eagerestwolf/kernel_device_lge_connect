...
//LGE_UPDATE_S	daheui.kim@lge.com For LCD KCAL
//extern int read_lcd_k_cal(char* buf);
#ifdef CONFIG_LGE_K_CAL 
extern int g_kcal[6];
#endif
...
//LGE_UPDATE_S daheui.kim@lge.com
#if defined(CONFIG_LGE_K_CAL)
...
#if defined(CONFIG_LGE_DISPLAY_MIPI_LGD_VIDEO_WVGA_PT) 
	cal_R = lcdtest_ascii_to_int(buf[0]) * 100 + lcdtest_ascii_to_int(buf[1])*10 + lcdtest_ascii_to_int(buf[2]);
	cal_G = lcdtest_ascii_to_int(buf[3]) * 100 + lcdtest_ascii_to_int(buf[4])*10 + lcdtest_ascii_to_int(buf[5]);
	cal_B= lcdtest_ascii_to_int(buf[6]) * 100 + lcdtest_ascii_to_int(buf[7])*10 + lcdtest_ascii_to_int(buf[8]);
#else
	cal_R = buf[0];
	cal_G = buf[1];
	cal_B = buf[2];
