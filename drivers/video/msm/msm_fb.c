...
//LGE_UPDATE_S daheui.kim@lge.com for init logo
...
#else
//This is to guarantee the dtv on/off when reference count is unstable.
#define IPRJ_DTV_CLK_ONOFF
#endif
...
//		msm_fb_blank_sub(FB_BLANK_UNBLANK, local_mfd->fbi, local_mfd->op_enable);		
		...
		printk("%s: buf %s, lcd on : %d, backlight level = 55 \n", __func__,buf, onoff);
	}
	else {
		msm_fb_set_backlight(local_mfd, 0);
...
static int msm_fb_probe(struct platform_device *pdev)
{
	...
	err = device_create_file(&pdev->dev, &dev_attr_mdp_kcal);//LGE_UPDATE heebae.song@lge.com for ATCMD
	if(err != 0)
...
*/
	...
	default:
		if (mfd->panel_power_on) { 
			...
			if (ret) // if fail to power off
				mfd->panel_power_on = curr_pwr_state;

			mfd->op_enable = TRUE;
		} else {  // if it's already off when powerdown is called
			if (pdata->power_ctrl)
