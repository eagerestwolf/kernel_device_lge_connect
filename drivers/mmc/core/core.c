...
int mmc_resume_host(struct mmc_host *host)
{
	...
	if(!strncmp(mmc_hostname(host),"mmc2",4))
	{
		...
		mmc_set_ios(host);
		printk("\n(IOS restore)\n%s: clock %uHz busmode %u powermode %u cs %u Vdd %u  width %u timing %u\n",mmc_hostname(host), ios_backup.clock, ios_backup.bus_mode,ios_backup.power_mode, ios_backup.chip_select, ios_backup.vdd,ios_backup.bus_width, ios_backup.timing);
        printk("\n%s: Data CRC Relative: %d Total %d",mmc_hostname(host),sdcard_relative_crc_c, sdcard_crc_c );
        printk("\n%s: Data TMO Relative: %d Total %d",mmc_hostname(host),sdcard_relative_data_c, sdcard_datatimeout_c );
        printk("\n%s: Data Ukn Relative: %d Total %d\n",mmc_hostname(host),sdcard_unknown, sdcard_unknown);
	}
