...
#include <linux/time.h> //sd card Test
...
unsigned int sdcard_relative_crc_c = 0;
EXPORT_SYMBOL(sdcard_relative_crc_c);
unsigned int sdcard_datatimeout_c = 0;
EXPORT_SYMBOL(sdcard_datatimeout_c);
unsigned int sdcard_relative_data_c = 0;
EXPORT_SYMBOL(sdcard_relative_data_c);
unsigned int sdcard_unknown = 0;
EXPORT_SYMBOL(sdcard_unknown);
unsigned int sdcard_unknown_relative = 0;
EXPORT_SYMBOL(sdcard_unknown_relative);
	...
	if (status & MCI_DATACRCFAIL) {
		if (!(data->mrq->cmd->opcode == MMC_BUS_TEST_W
            ...
            if (host->mmc->index == 2){
                start_time = myclock();
                    sdcard_crc_c++;
               if((start_time - end_time > 3000) && end_time !=0)
                    sdcard_relative_crc_c++;
            }
