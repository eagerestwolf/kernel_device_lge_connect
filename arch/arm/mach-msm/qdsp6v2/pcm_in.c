...
/*
	pr_info("NXP:%s() Tx_timestamp Avail=[%4lu] Secs=[%10lu] nSecs=[%9lu] bytes_acked(%lu) bytes_read(%lu) at %d in pcm_in.c\n", \
...
/* Copyright 2011 NXP Software, begin */
	...
	pcm->bytes_acked = 0;
/*
	pr_info("NXP: %s(): Tx_timestamp Avail=[%5lu] Secs=[%6lu] nSecs=[%9lu] bytes_read(%lu) bytes_acked(%lu) at %d in qdsp6v2/pcm_out.c\n", \
			__func__, pcm->tstamp.avail, pcm->tstamp.time.tv_sec, pcm->tstamp.time.tv_nsec, pcm->bytes_read, pcm->bytes_acked, __LINE__);
*/
/* Copyright 2011 NXP Software, end */
#endif
...
//		pr_info("NXP: %s(): cmd(AUDIO_PCM_HTIMESTAMP) at %d in qdsp6v2/pcm_in.c\n", __func__, __LINE__);
...
//	pr_info(" NXP: %s(): count(%d) bytes_read(%lu) at %d in qdsp6v2/pcm_in.c\n", __func__, count, pcm->bytes_read, __LINE__);
