...
#ifdef CONFIG_LG_DRIVING_CURRENT_SD
extern bool drving_current_ctrl;
#endif
...
#ifdef CONFIG_LG_DRIVING_CURRENT_SD
static struct msm_sdcc_pad_drv_cfg sdc3_pad_on_drv_cfg_changed[] = {
...
#endif
#ifdef CONFIG_LG_POWER_CUT_DOWN
	...
	if (enable) {
		...
		for (n = 0; n < curr->pad_drv_data_size; n++) {
			if (curr->sdio_lpm_gpio_cfg) {
...
#ifdef CONFIG_LG_DRIVING_CURRENT_SD
                if((dev_id == 3) && (drving_current_ctrl == true)){
                msm_tlmm_set_hdrive(sdc3_pad_on_drv_cfg_changed[n].drv,
				sdc3_pad_on_drv_cfg_changed[n].drv_val);

                printk("mmc%d: sandisk drive strength changed :%d %d \n", dev_id -1, 
                sdc3_pad_on_drv_cfg_changed[n].drv,sdc3_pad_on_drv_cfg_changed[n].drv_val);
            }
            else{
#endif
                ...
                printk("mmc%d: drive strength changed :%d %d \n", dev_id -1, 
                curr->pad_drv_on_data[n].drv, curr->pad_drv_on_data[n].drv_val);
#ifdef CONFIG_LG_DRIVING_CURRENT_SD                
                }            
#endif            
		}
